...
public class Bayes {
	...
	public int possibilityBayes(String txt) {
		...
		masterpos.forEach(
				(key, value) -> master.merge(key, value, (oldValue, newValue) -> newValue = oldValue + newValue));
		...
		for (int y = 0; y < temp.size(); y++) {
			...
			if (masterpos.get(temp.get(y)) == null) {
				...
				ppos = ppos * masterpos.get(temp.get(y)) / master.get(temp.get(y)) * (cpos - masterpos.get(temp.get(y)))
						/ (example.size() - master.get(temp.get(y)));
				pneg = pneg * masterneg.get(temp.get(y)) / master.get(temp.get(y)) * (cneg - masterneg.get(temp.get(y)))
						/ (example.size() - master.get(temp.get(y)));

			}
			if (!flagpos) {
				if (!(masterneg.get(temp.get(y)) == null)) {
					ppos = ppos * 1 / master.get(temp.get(y)) * (cpos - 1)
							/ (example.size() - master.get(temp.get(y)) + 2);
					pneg = pneg * masterneg.get(temp.get(y)) / master.get(temp.get(y))
							* (cneg - masterneg.get(temp.get(y))) / (example.size() - master.get(temp.get(y)));
				}

			}
			if (!flagneg) {
				if (!(masterpos.get(temp.get(y)) == null)) {
					pneg = pneg * 1 / master.get(temp.get(y)) * (cneg - 1)
							/ (example.size() - master.get(temp.get(y)) + 2);
					ppos = ppos * masterpos.get(temp.get(y)) / master.get(temp.get(y))
							* (cpos - masterpos.get(temp.get(y)) + 1) / (example.size() - master.get(temp.get(y)));
				}
	...
	public void loadAll(int k, String path) throws IOException {
		int i = 0;
		List<String> filenames = Files.list(Paths.get(path + "\\train\\neg")).filter(Files::isRegularFile)
				.map(p -> p.getFileName().toString()).collect(Collectors.toList());
		while (i < k / 2) {
			HashMap<String, Integer> temporary;
			BufferedReader reader = new BufferedReader(new FileReader(path + "\\train\\neg\\" + filenames.get(i)));
			String tempor = reader.readLine();
			temporary = words(tempor, 1);
			temporary.forEach((key, value) -> masterneg.merge(key, value,
		...
		List<String> filenames2 = Files.list(Paths.get(path + "\\train\\pos")).filter(Files::isRegularFile)
				.map(p -> p.getFileName().toString()).collect(Collectors.toList());
		// read the positive reviews
		int j = 0;
		while (j < k / 2) {
			BufferedReader reader = new BufferedReader(new FileReader(path + "\\train\\pos\\" + filenames2.get(j)));
			String tempor = reader.readLine();
			HashMap<String, Integer> temporary2;
			temporary2 = words(tempor, 0);
			temporary2.forEach((key, value) -> masterpos.merge(key, value,
					(oldValue, newValue) -> newValue = oldValue + newValue));
	...
	public void loadTest(int k, String path) throws IOException {
		int i = 0;
		List<String> filenames = Files.list(Paths.get(path + "\\test\\neg")).filter(Files::isRegularFile)
				.map(p -> p.getFileName().toString()).collect(Collectors.toList());

		while (i < k / 2) {
			HashMap<String, Integer> temporary;
			BufferedReader reader = new BufferedReader(new FileReader(path + "\\test\\neg\\" + filenames.get(i)));
			String tempor = reader.readLine();
			test.put(tempor, 1);
			i++;
		}
		List<String> filenames2 = Files.list(Paths.get(path + "\\test\\pos")).filter(Files::isRegularFile)
				.map(p -> p.getFileName().toString()).collect(Collectors.toList());
		// read the positive reviews
		int j = 0;
		while (j < k / 2) {
			BufferedReader reader = new BufferedReader(new FileReader(path + "\\test\\pos\\" + filenames2.get(j)));
			String tempor = reader.readLine();
			test.put(tempor, 0);
			j++;

...
public class Algorithm {
    ...
    public String ID3_test() {
        ...
        for (int w = 0; w < test.size(); w++) {
            for (int u = 0; u < train_attributes.size(); u++) {
                //me bash to tree map
                if (test2.get(w).contains(train_attributes.get(u))) {//gia to threshold allazoyme tis pithanothtes parakatw
    ...
    public int findMajority(HashMap<String, Integer> majority) {
       ...
       public HashMap<String, Integer> loadAll( int k,String path) throws IOException {
        int i = 0;
        ...
        List<String> filenames2 = Files.list(Paths.get(path + "\\train\\pos"))
                .filter(Files::isRegularFile)
                .map(p -> p.getFileName().toString())
                .collect(Collectors.toList());
    ...
    public void loadTest(int k,String path) throws IOException {
        ...
        List<String> filenames2 = Files.list(Paths.get(path + "\\test\\pos"))
                .filter(Files::isRegularFile)
                .map(p -> p.getFileName().toString())
                .collect(Collectors.toList());
        //read the positive reviews
